...
const build = new Builder({ lib: 'biota.core', path: 'function', version: packagejson.version });
...
const checkSchema = (_, options) => ({
  schema: schema.validate.ValidateThrow.response(codes.data_validation_failed.name, options, {
    type: 'schema',
    schema: schema.function_.Schema.udfName(),
    options: {},
  }),
});
...
export const function_: types.BiotaBuilderMethodOutputAPIKeyed = build.methods({
  ...
  Insert: {
    ...
    annotate: 'insert',
    query(_, options) {
      return q.CreateFunction(q.Merge(options, { data: q.Var('data') }));
    },
    action: 'insert',
  },
  Update: {
    ...
    annotate: 'update',
    query(name, options) {
      return q.Update(q.Function(name), q.Merge(options, { data: q.Var('data') }));
    },
    action: 'update',
  },
  Upsert: {
    ...
    query(name, options) {
      return q.If(
        q.Select('valid', schema.validate.Validate.response(q.Function(name), { type: 'function' }), false),
        function_.Update.response(name, options),
        function_.Insert.response(name, options),
      );
    },
  },
  Replace: {
    ...
    before(name, options) {
      return {
        ...updateInputs(name, options),
        ...checkRefExists(name),
        doc: function_.Get.response(name),
        data: q.Select('data', q.Var('doc'), {}),
      };
    },
    annotate: 'replace',
    query(name, options) {
      return q.Replace(q.Function(name), q.Merge(q.Merge({ name }, options), { data: q.Var('data') }));
    },
    action: 'replace',
  },
  Repsert: {
    ...
    query(name, options) {
      return q.If(
        q.Select('valid', schema.validate.Validate.response(q.Function(name), { type: 'function' }), false),
        function_.Replace.response(name, options),
        function_.Insert.response(name, options),
      );
    },
  },
  Delete: {
    ...
    annotate: 'delete',
    query(name) {
      return function_.Upsert.response(name, { data: q.Var('data') });
    },
    action: 'delete',
  },
  Restore: {
  ...
  Forget: {
    ...
    after(name) {
      return {
        response: q.Delete(q.Function(name)),
      };
    },
  },
  Drop: {
    ...
    query(name) {
      return q.If(
        ...
        function_.Forget.response(name),
  ...
  ExpireIn: {
    ...
    defaults: [null, 0],
    query(name, delay) {
      return function_.Expire.response(name, q.TimeAdd(q.Now(), q.ToNumber(delay), 'milliseconds'));
    },
  },
  ExpireNow: {
    name: 'ExpireNow',
    ...
    defaults: [null],
    query(name) {
      return function_.Expire.response(name);
    },
  },
});
