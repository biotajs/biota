...
const build = new Builder({ lib: 'biota.core', path: 'database', version: packagejson.version });
...
const checkSchema = (_, options) => ({
  schema: schema.validate.ValidateThrow.response(codes.data_validation_failed.name, options, {
    type: 'schema',
    schema: schema.database.Schema.udfName(),
    options: {},
  }),
});
...
export const database: types.BiotaBuilderMethodOutputAPIKeyed = build.methods({
  ...
  Insert: {
    ...
    annotate: 'insert',
    query(_, options) {
      return q.CreateDatabase(q.Merge(options, { data: q.Var('data') }));
    },
    action: 'insert',
  },
  Update: {
    ...
    annotate: 'update',
    query(name, options) {
      return q.Update(q.Database(name), q.Merge(options, { data: q.Var('data') }));
    },
    action: 'update',
  },
  Upsert: {
    ...
    query(name, options) {
      return q.If(
        q.Select('valid', schema.validate.Validate.response(q.Database(name), { type: 'database' }), false),
        database.Update.response(name, options),
        database.Insert.response(name, options),
      );
    },
  },
  Replace: {
    ...
    before(name, options) {
      return {
        ...updateInputs(name, options),
        ...checkRefExists(name),
        doc: database.Get.response(name),
        data: q.Select('data', q.Var('doc'), {}),
      };
    },
    annotate: 'replace',
    query(name, options) {
      return q.Replace(q.Database(name), q.Merge(q.Merge({ name }, options), { data: q.Var('data') }));
    },
    action: 'replace',
  },
  Repsert: {
    ...
    query(name, options) {
      return q.If(
        q.Select('valid', schema.validate.Validate.response(q.Database(name), { type: 'database' }), false),
        database.Replace.response(name, options),
        database.Insert.response(name, options),
      );
    },
  },
  Delete: {
    ...
    annotate: 'delete',
    query(name) {
      return database.Upsert.response(name, { data: q.Var('data') });
    },
    action: 'delete',
  },
  Restore: {
  ...
  Forget: {
    ...
    after(name) {
      return {
        response: q.Delete(q.Database(name)),
      };
    },
  },
  Drop: {
  ...
  ExpireIn: {
    ...
    defaults: [null, 0],
    query(name, delay) {
      return database.Expire.response(name, q.TimeAdd(q.Now(), q.ToNumber(delay), 'milliseconds'));
    },
  },
  ExpireNow: {
    name: 'ExpireNow',
    ...
    defaults: [null],
    query(name) {
      return database.Expire.response(name);
    },
  },
});
